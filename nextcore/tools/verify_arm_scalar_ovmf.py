...
def main():
    ...
    source=Path(__file__).resolve().parent.parent / "artifacts/arm-memory-provider-20260909/arm64_scalar_probe.S"
    ...
    for index,(name,definition) in enumerate(CASES[:1] if args.negative_control else CASES):
        ...
        if index<4:
            ...
            if definition in ("PROBE_DATA_LOAD","PROBE_DATA_STORE"):
                ...
                # A64 EC=0 requires IL=1 and ISS=0; reject the historical zero-IL result.
                checks.update(undefined_status=e.get("status")==8,undefined_esr=exc.get("esr")==0x02000000,
