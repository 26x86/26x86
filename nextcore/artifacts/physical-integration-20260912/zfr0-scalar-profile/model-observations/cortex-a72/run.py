...
qemu=shutil.which(a.qemu);assert qemu;ep=out/'qmp.sock';cmd=[qemu,'-machine','virt,virtualization=off,secure=off','-cpu','cortex-a72','-m','128','-display','none','-serial','none','-monitor','none','-nic','none','-qmp',f'unix:{ep},server=on,wait=off','-kernel',str(out/'probe.elf')];commands.append(cmd)
