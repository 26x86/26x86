...
out=BASE/'gates'/(args.phase if args.attempt==1 else args.phase+'-r'+str(args.attempt))/name
...
if args.phase=='post':
    published=json.loads((BASE/(name+'-published.json')).read_text())
    for ref in ['HEAD',meta['release_tag']]:
        current=subprocess.run(['git','rev-parse',ref],cwd=repo,capture_output=True,check=True).stdout.decode().strip()
        assert current==published['head']
    assert not subprocess.run(['git','status','--porcelain'],cwd=repo,capture_output=True,check=True).stdout
for row in inventory:
    b=(subprocess.run(['git','show','HEAD:'+row['path']],cwd=repo,capture_output=True,check=True).stdout
       if args.phase=='post' else (repo/row['path']).read_bytes())
...
result={'module':name,'phase':args.phase,'attempt':args.attempt,'source_commit':meta['source_commit'],'inventory_files_verified':len(inventory),'inventory_basis':'committed Git blobs and clean worktree' if args.phase=='post' else 'prepared bytes','commands':[],'passed':False,'guest_execution_claimed':False}
