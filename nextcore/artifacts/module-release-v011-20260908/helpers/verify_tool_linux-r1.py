...
parser=argparse.ArgumentParser();parser.add_argument('phase',choices=['pre','post']);args=parser.parse_args()
record=pathlib.Path(__file__).resolve().parent/'tool-v012'
prepared=json.loads((record/'prepared.json').read_text())
base=pathlib.Path('/tmp/nextcore-tool-v012-'+args.phase+'-20260908')
...
def run(argv,cwd=None,timeout=300):
 ...
 n=len(commands)
 (record/f'{args.phase}-{n}.stdout').write_bytes(result.stdout)
 (record/f'{args.phase}-{n}.stderr').write_bytes(result.stderr)
...
result={'phase':args.phase,'source_commit':prepared['source_commit'],'expected_head':prepared['head'],'single_repo_parent':str(base),'passed':False,'commands':commands,'guest_execution_claimed':False}
try:
 ...
 (record/(args.phase+'-linux-receipt.json')).write_text(json.dumps(result,indent=2)+'\n')
