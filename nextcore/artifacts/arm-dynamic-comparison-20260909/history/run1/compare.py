...
FREEZE_SHA="72f5e7e58ad42c1428cbced6c261d37783061c13b45639a465faada7d51dc7d3"
RESULT=["status","abi","size","provider","fetches","data_requests","completed","last_address","guest_far",
        "reply_result","reply_fault","reply_fsc","reply_level","reply_address","reply_esr",
...
def compare(case,actual,ram,expected_ram):
 ...
 status=13 if success else 16 if kind==1 else 17
 ...
 checks={
  "supported_boundary":r["status"]==status and cpu[47]==status,
  ...
  "guest_esr":cpu[36]==o["esr"],
  "guest_far":cpu[37]==o["far"] and r["guest_far"]==o["far"],
  "guest_elr":cpu[38]==o["elr"],
  "guest_spsr":cpu[39]==(0 if success else o["spsr"]),
 ...
 if not success:
  ...
  checks["unaltered_hvc_fetched"]=(r["reply_result"],r["reply_fault"])==(0,0)
 return {"name":case["name"],"expected_native_status":status,"expected_native_retired":retirements,
...
def main():
 ...
 tool_inputs=["compare.py","observer.c","native_replay.rs","contract.md"]
 ...
 result={"schema":"nextcore.bp34-captured-arm-native-dynamic-comparison.v1","passed":passed,"case_count":6,"cases":cases,
    ...
    "hvc_supported":False,"hvc_boundary":"Original HVC0x34 remains at codeVA+40. Native success comparison stops at its unsupported nonretired system boundary; actual Arm executes HVC to EL2.",
    "entry_adaptations":{"hcr":"Captured RW-only0x80000000 checked; inactive native/service HCR0, with no stage2 or TGE bits removed.","scr":"Uncaptured EL3 state represented as inactive0; no equality claimed.","pstate":"Exact captured3c5 retained; entry ERET/SPSR source establishes this state and payload has no flag/state writes. No masking.","stack":"Original SP_EL1 symbol from ELF/start.S setup; no payload SP access. EL2 setup/handlers are not executed by native.","other_registers":"Only authored x0-x3 and zeroed x10-x14 are initialized/compared; unobserved caller registers are not claimed identical."},
