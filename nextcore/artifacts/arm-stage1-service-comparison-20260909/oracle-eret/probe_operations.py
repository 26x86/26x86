...
def cases(include_known_qemu_pc_priority=False):
 if include_known_qemu_pc_priority:raise ValueError('Known PC-priority deviations remain in original separate bundle.')
 return json.loads((Path(__file__).resolve().parent/'selected_cases.json').read_text())
...
def main():
 ...
 source_paths=[source/name for name in ['operation_start.S','operation_oracle.c','oracle.ld','probe_operations.py','selected_cases.json']]
